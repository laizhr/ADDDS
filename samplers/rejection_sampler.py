...
def get_rgo_sampling(xk, eta, log_prob, device, threshold, minimizer=None):
    ...
    total_proposals += accepted_samples.numel()
    ...
    n_accepted = acc_idx.sum().item()
    acceptance_rate = n_accepted / (total_proposals + 1e-9)
    return xk, acc_idx, acceptance_rate
...
def get_samples(y, eta, distribution : utils.densities.Distribution, num_samples, device,threshold=1e-3):
    ...
    samples, accepted_idx, acc_rate = get_rgo_sampling(yk,eta,distribution.log_prob,device, threshold, minimizer=distribution.potential_minimizer)
    samples = samples.reshape((n, -1, d))
    accepted_idx = accepted_idx.reshape((n,-1,d))

    return samples, accepted_idx, acc_rate