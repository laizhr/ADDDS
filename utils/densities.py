...
class MixtureDistribution(Distribution):
    ...
    def _log_prob(self, x):
        ...
        log_probs_shifted = log_probs - max_log_prob.detach()
        exp_log_probs_shifted = torch.exp(log_probs_shifted)
        sum_exp = torch.sum(exp_log_probs_shifted, dim=-1, keepdim=True)
        log_dens = max_log_prob + torch.log(sum_exp + 1e-9)
    ...
    def _grad_log_prob(self, x):
        ...
        grads_list = []
        for i in range(self.n):
            log_probs_list.append(log(self.c[i]) + self.distributions[i].log_prob(x))
            grads_list.append(self.distributions[i].grad_log_prob(x))

        log_probs = torch.cat(log_probs_list, dim=-1)
        all_grads = torch.stack(grads_list, dim=-2)
        weights = torch.nn.functional.softmax(log_probs, dim=-1)
        grad = torch.sum(weights.unsqueeze(-1) * all_grads, dim=-2)
