...
def sample(config, distribution=None):
    ...
    acc_rate_std = 0.0
    try:
        estimator = model.__self__ if hasattr(model, '__self__') else model

        if hasattr(estimator, 'avg_acceptance_rate'):
            acc_rate = estimator.avg_acceptance_rate
            acc_rate_std = getattr(estimator, 'std_acceptance_rate', 0.0)
            print(f"Acceptance Rate: {acc_rate:.4f} ± {acc_rate_std:.4f}")

    except Exception:
        pass
    config.last_run_acc_rate = acc_rate
    config.last_run_acc_rate_std = acc_rate_std
